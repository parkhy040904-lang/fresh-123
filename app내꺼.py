  ...
  try {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: produce + '를 주재료로 한 실제 한국 가정식 레시피를 알려주세요.\\n\\n조건:\\n- 네이버 블로그나 유튜브에서 실제로 검색되는 유명한 요리여야 합니다\\n- 재료는 2인분 기준으로 정확한 계량(큰술/작은술/g/개 등) 포함\\n- 조리법은 불 세기·시간·온도를 포함해 단계별로 구체적으로\\n- 아래 형식으로만 답하고 다른 말은 절대 쓰지 마세요\\n\\n요리이름: (실제 존재하는 요리명)\\n조리시간: (총 소요시간)\\n재료: (재료1 양, 재료2 양, ... 형태로 나열)\\n조리법: (1. 첫번째 단계. 2. 두번째 단계. 형태로 상세하게)'
        }]
      })
    });
