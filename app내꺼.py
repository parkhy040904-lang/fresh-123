  ...
  const run = async () => {
    ...
    try {
      ...
      const raw = json.choices[0].message.content
        ...
        .trim();
      if (raw.startsWith('분석불가')) {
        const reason = raw.includes(':') ? raw.split(':').slice(1).join(':').trim() : '과일이나 채소 사진을 올려주세요';
        document.getElementById('remo').textContent = '🚫';
        document.getElementById('rname').textContent = '분석 불가';
        document.getElementById('rscore').textContent = reason;
        document.getElementById('bfill').style.width = '0%';
        document.getElementById('scoreGrid').innerHTML = '';
