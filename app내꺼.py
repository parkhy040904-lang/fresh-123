...
async function analyze(src) {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (!checkRes.ok) throw new Error('API 오류: ' + checkRes.status);
      const checkJson = await checkRes.json();
      const checkRaw = checkJson.choices[0].message.content.trim().toUpperCase();
      if (!checkRaw.startsWith('YES')) {
        document.getElementById('remo').textContent = '🚫';
        document.getElementById('rname').textContent = '분석 불가';
        document.getElementById('rscore').textContent = '과일이나 채소 사진을 올려주세요';
        document.getElementById('bfill').style.width = '0%';
        document.getElementById('scoreGrid').innerHTML = '';
        document.getElementById('rtags').innerHTML = '<span class="tag y">인식 불가</span>';
        document.getElementById('rtip').textContent = '🥦 과일이나 채소 사진을 올려주세요!';
        document.getElementById('btnRow').style.display = 'none';
        return;
      }
